...
# Try both seasons to cover leagues on different calendars
SEASONS = [2025, 2026]
...
def get_fixtures_for_date(date_str):
    """Fetch all fixtures for a given date across all monitored leagues and seasons."""
    ...
    for season in SEASONS:
        for league_id in LEAGUE_IDS:
            try:
                ...
                if fixtures:
                    print(f"  [INFO] League {league_id} season {season}: {len(fixtures)} fixtures")
            except Exception as e:
                print(f"[WARN] Failed league {league_id} season {season}: {e}")

    print(f"[INFO] Total unique fixtures found: {len(all_fixtures)}")
    return all_fixtures
...
def run(target_date=None):
    ...
    if not fixtures:
        msg = (
            ...
            f"This may be a rest day across monitored leagues."
        )
        send_telegram(msg)
        return
